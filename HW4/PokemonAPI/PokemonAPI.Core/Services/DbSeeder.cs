...
public class DbSeeder : IDbSeeder
{
    ...
    public async Task SeedAsync(CancellationToken cancellationToken)
    {
        ...
        .GetAsync($"https://pokeapi.co/api/v2/pokemon?limit=1302", cancellationToken: cancellationToken)
        ...
        foreach (var poke in basePokes.Pokes)
        {
            ...
            using var pokeInfo = await _httpClient
                .GetAsync($"https://pokeapi.co/api/v2/pokemon?limit=1302/{poke.PokeName.ToLower()}", cancellationToken)
    ...
    {
        using var response = await _httpClient.GetAsync("https://pokeapi.co/api/v2/type?limit=20", cancellationToken);
    ...
    private async Task SeedAbilitiesAsync(CancellationToken cancellationToken)
    {
        ...
        var response = await _httpClient.GetAsync("https://pokeapi.co/api/v2/ability?limit=367", cancellationToken);
    ...
    private async Task SeedMovesAsync(CancellationToken cancellationToken)
    {
        ...
        var response = await _httpClient.GetAsync("https://pokeapi.co/api/v2/move?limit=937", cancellationToken);
